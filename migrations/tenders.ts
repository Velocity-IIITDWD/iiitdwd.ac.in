  ...
  try {
    ...
    for (const tender of tenders) {
      transaction.create({
        ...
        corrections: tender.corrections?.map((correction: { title: string; link: string; }) => ({
          title: correction.title,
          link: correction.link,
        })),
        documents: tender.documents?.map((document: { title: string; link: string; }) => ({
    ...
    console.error('Error migrating tenders:', error);
  }
}
