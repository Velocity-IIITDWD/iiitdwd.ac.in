...
interface ProfileProps {
  ...
  const Profile = ( {params}) => {
    ...
    const profile: ProfileProp | undefined = ProfileProps.find(profileProp => profileProp.id == profileId);
    console.log(profile?.content.head.name)
    if (!profile) {
      return <div></div>;
    }
    const areaofinterest_array = profile.content.body.interest_areas.map((arr) => <li>{arr}</li> )
        ...
        <>
            ...
            <div className="bg-dwd-secondary2 flex flex-row justify-between relative px-12 py-3">
                ...
                <div className="bg-dwd-primary flex justify-center  px-2 py-2 rounded">
                    <a href={profile.content.head.profile_pdf} className="align-middle bg-dwd-primary flex justify-center text-background rounded">
                    View Profile
                    </a>
                </div>
            </div>
           ...
           <div className="bg-background flex flex-col md:flex-row lg:flex-row xl:flex-row gap-6 2xl:flex-row  justify-between relative py-12 px-12 space-x-12">
                ...
                <div className="flex flex-col space-y-8">
                    <div>
                        <div className="flex flex-col">
                        <div className="font-bold text-xl border-b-2 border-black">Profile</div>
                        <div className="pl-2 py-4">{profile.content.body.profile_text}</div>
                        </div>
                        
                    </div>
                    <div>
                        <div className="flex flex-col  py-20"> 
                        <div className="font-bold text-xl border-b-2 border-black">Area Of Interest</div>
                        <div><ul className="list-disc pl-10 py-4">{areaofinterest_array}</ul></div>
                        </div>
