...
const List = ({ ll }: ListProps) => {
  ...
  function qwe() {
    ...
    const f_array = final.map((arr) => (
      <li
        key={arr.id}
        className="bg-dwd-secondary2 hover:bg-dwd-secondary1 flex grid flex-none justify-center text-background  px-6 py-6 rounded-lg w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 min-[3200px]:w-[12%] mb-4 "
      >
        <Link href={`/faculty/${arr.id}`}>
          <div className="min-[3600px]:w-auto flex flex-col gap-4 min-[3600px]:flex-row">
            ...
            <div className='min-[3600px]:w-1/2 flex flex-col justify-center'>
  ...
  return (
    <>
      <div className="flex flex-col justify-center py-0  m-4 gap-4 ">
        ...
        <div className='flex'>
        <ul className="bg-background flex flex-wrap mx-12 gap-8 content-start ">{f_array}</ul>
        </div>
      </div>
    </>
  );
