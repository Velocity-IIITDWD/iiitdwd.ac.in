...
const List = ({ ll }: ListProps) => {
  const f_array = ll.map((arr) => (
    <li
      key={arr.id}
      className="bg-dwd-secondary2 hover:bg-dwd-secondary1 flex-col flex-none justify-center text-background px-6 py-6 rounded-lg w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 mb-4 "
    >
  ...
  return (
    <>
      <div className="flex flex-col justify-center py-12 m-2 ml-4 gap-4 ">
        ...
        <ul className="bg-background flex flex-wrap  mx-12 gap-8 ">
          {f_array}
        </ul>
      </div>
    </>
